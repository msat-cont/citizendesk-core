...
bp_feed_sms_reply = Blueprint('bp_feed_sms_reply', __name__)

def setup_blueprints(app):
    app.register_blueprint(bp_feed_sms_reply)
    return

@bp_feed_sms_reply.route('/feeds/sms/reply/', defaults={}, methods=['POST'], strict_slashes=False)
def feed_sms_reply_one_post():
    ...
    res = process.do_post_reply(mongo_dbs.get_db().db, sms_gateway_url, sms_gateway_key, message, report_id, user_id, language, sensitive, client_ip)
